#include "Renderer.h"

Renderer::Renderer(std::string title, int w, int h)
{
    Create(title, w, h);
}

Renderer::~Renderer()
{
    glfwDestroyWindow(m_window);
    glfwTerminate();
}

void Renderer::Create(std::string title, int w, int h)
{
    p.m_width = w;
    p.m_height = h;
    p.m_title = title;

    GLFW_Init();
    GLFW_CreateWindow(title, w, h);
    GLAD_Load();
    GLFW_SetFrameBufferSizeCallback();

    UpdateViewport({ 0, 0 }, { w, h });
    PrepareDrawing();
}

void Renderer::UpdateViewport(const vi2& pos, const vi2& size)
{
    glViewport(pos.x, pos.y, size.x, size.y);
}

void Renderer::ClearBuffer(pixel p, bool depth)
{
    glClearColor(float(p.r) / 255.0f, float(p.g) / 255.0f, float(p.b) / 255.0f, float(p.a) / 255.0f);
    glClear(GL_COLOR_BUFFER_BIT);
    if (depth) glClear(GL_DEPTH_BUFFER_BIT);

}

void Renderer::PrepareDrawing(bool depth, bool cullface)
{
    if (depth)
    {
        glEnable(GL_DEPTH_TEST);
        glDepthFunc(GL_LESS);
    }

    if (cullface)
    {
        glEnable(GL_CULL_FACE);
        glCullFace(GL_BACK);
    }

    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
}

void Renderer::DisplayFrame()
{
    glfwSwapBuffers(m_window);
}

int Renderer::Width()
{
    return p.m_width;
}

int Renderer::Height()
{
    return p.m_height;
}

GLFWwindow* Renderer::GetWindow()
{
    return m_window;
}

void Renderer::Close()
{
    glfwSetWindowShouldClose(m_window, true);
}

bool Renderer::ShouldClose()
{
    return glfwWindowShouldClose(m_window);
}

void Renderer::GLFW_Init()
{
    if (!glfwInit())
    {
        std::cout << "GLFW Initialization Failed" << std::endl;
        glfwTerminate();
    }
    // OpenGL version
    glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 3);
    glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, 3);
    // Core Profile: No backward compatibility
    glfwWindowHint(GLFW_OPENGL_PROFILE, GLFW_OPENGL_CORE_PROFILE);
    // Enable Forward Compatibility
    glfwWindowHint(GLFW_OPENGL_FORWARD_COMPAT, GL_TRUE);
    glfwWindowHint(GLFW_RESIZABLE, false);
    printf("GLFW %i.%i.%i\n", GLFW_VERSION_MAJOR, GLFW_VERSION_MINOR, GLFW_VERSION_REVISION);
}

void Renderer::GLFW_CreateWindow(std::string title, int w, int h)
{
    m_window = glfwCreateWindow(w, h, title.c_str(), nullptr, nullptr);
    if (!m_window)
    {
        std::cout << "GLFW Window Initialization Failed" << std::endl;
        glfwTerminate();
    }
    glfwMakeContextCurrent(m_window);
    glfwSwapInterval(0); // Remove Framecap
}

void Renderer::GLAD_Load()
{
    if (!gladLoadGLLoader((GLADloadproc)glfwGetProcAddress))
    {
        std::cout << "GLAD Initialization Failed" << std::endl;
        glfwTerminate();
    }
}

void Renderer::GLFW_SetFrameBufferSizeCallback()
{
    glfwSetWindowUserPointer(m_window, &p);
    glfwSetFramebufferSizeCallback(m_window, [](GLFWwindow* window, int width, int height)
    {
        param* p = static_cast<param*>(glfwGetWindowUserPointer(window));
        p->m_width = width;
        p->m_height = height;
        glViewport(0, 0, width, height);
    });
}
